...
**Phase 2 — Oracle Setup.** Generate an OOB token via `oob-helper.sh`, producing a callback URL on `poc-harness.com`. Inject this URL into every discovered parameter. Watch for callbacks.
...
Blind vulnerability testing requires out-of-band confirmation. The `oob-helper.sh` script wraps the poc-harness.com OOB server:

```bash
OOB_SERVER="https://poc-harness.com"
