...
graph LR
    ...
    style R1 fill:#264653,stroke:#2a9d8f,color:#fff
    style R2 fill:#2a9d8f,stroke:#e9c46a,color:#fff
    style R3 fill:#e9c46a,stroke:#000,color:#000
    style R4 fill:#f4a261,stroke:#000,color:#000
    style R5 fill:#e76f51,stroke:#fff,color:#fff
```
...
flowchart TD
    ...
    RESULT -->|"BLOCKED"| BINARY["Phase 3: BINARY SEARCH<br/>Decompose → test components<br/>→ identify WAF triggers"]
    RESULT -->|"REFLECTED"| CSP{"CSP<br/>blocking?"}
    CSP -->|Yes| CSP_BYPASS["Try CSP bypasses"]
    CSP -->|No| BINARY

    BINARY --> MUTATE["Phase 4: MUTATION LOOP<br/>Composer generates bypass<br/>based on learned blocks"]

    MUTATE --> MRESULT{Result?}
    MRESULT -->|"EXECUTED"| SUCCESS
    MRESULT -->|"BLOCKED"| LEARN["Binary search again<br/>Update knowledge"]
    LEARN --> PIVOT{"4+ common<br/>tags blocked?"}
    PIVOT -->|Yes| ENUM["Phase 5: ENUMERATE<br/>Fuzz all tags → events"]
    PIVOT -->|No| MUTATE

    MRESULT -->|"Exhausted"| ENUM

    ENUM --> COMBINE["Phase 6: COMBINE<br/>allowed_tag + allowed_event<br/>+ allowed_function"]
    COMBINE --> FINAL{Result?}
    FINAL -->|"EXECUTED"| SUCCESS
    FINAL -->|"All exhausted"| FAIL(["Not Vulnerable<br/>(with current techniques)"])

    style SUCCESS fill:#00ff88,stroke:#000,color:#000
    style FAIL fill:#ff4444,stroke:#fff,color:#fff
    style BINARY fill:#2a9d8f,stroke:#fff,color:#fff
    style MUTATE fill:#e76f51,stroke:#fff,color:#fff
    style ENUM fill:#f4a261,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    PROBE["Send probe: enigma_probe_123"] --> FIND{"Where does<br/>probe appear<br/>in response?"}

    FIND -->|Between tags| HTML["html_content<br/>Baseline: img onerror=alert#40;1#41;"]
    FIND -->|Inside attribute value| ATTR["html_attribute<br/>Baseline: break quote + onfocus + autofocus"]
    FIND -->|Inside JS string| SCRIPT["script_string<br/>Baseline: break string + alert"]
    FIND -->|Inside href/src| HREF["href_attribute<br/>Baseline: javascript:alert#40;1#41;"]
    FIND -->|Inside SVG| SVG["svg_context<br/>Baseline: svg onload=alert#40;1#41;"]
    FIND -->|Inside event handler| EVENT["event_handler<br/>Baseline: close call + alert"]
    FIND -->|Hidden input field| HIDDEN["hidden_input<br/>Baseline: accesskey + onclick"]
    FIND -->|Template expression| FRAMEWORK["framework_template<br/>Baseline: constructor chain"]

    style HTML fill:#264653,color:#fff
    style ATTR fill:#2a9d8f,color:#fff
    style SCRIPT fill:#e9c46a,color:#000
    style HREF fill:#f4a261,color:#000
    style SVG fill:#e76f51,color:#fff
    style EVENT fill:#9b59b6,color:#fff
    style HIDDEN fill:#3498db,color:#fff
...
```mermaid
flowchart LR
    BLOCKED["Blocked payload:<br/>img onerror=alert#40;1#41;"] --> DECOMPOSE["Decompose into<br/>components"]
    ...
    T1 -->|"No"| OK1["✅ Tag allowed"]
    T2 -->|"No"| OK2["✅ Event allowed"]
    T3 -->|"Yes"| BLOCK3["❌ 'alert' is trigger<br/>→ KEYWORD cascade"]
    T4 -->|"Yes"| BLOCK4["❌ '(' is trigger<br/>→ PAREN cascade"]
    T5 -->|"No"| OK5["✅ ) allowed"]
    T6 -->|"No"| OK6["✅ > allowed"]

    style BLOCK3 fill:#e74c3c,color:#fff
    style BLOCK4 fill:#e74c3c,color:#fff
    style OK1 fill:#2ecc71,color:#000
    style OK2 fill:#2ecc71,color:#000
    style OK5 fill:#2ecc71,color:#000
    style OK6 fill:#2ecc71,color:#000
```
...
```mermaid
flowchart TD
    BLOCKED["Parentheses blocked<br/>Cannot call alert#40;1#41;"] --> B1{"Backticks<br/>allowed?"}

    B1 -->|Yes| TICK["Tagged template<br/>alert#96;1#96;"]
    B1 -->|No| B2{"throw<br/>allowed?"}

    B2 -->|Yes| THROW["Throw coercion<br/>throw onerror=alert,1"]
    B2 -->|No| B3{"+ operator<br/>allowed?"}

    B3 -->|Yes| VALUEOF["Implicit type coercion<br/>valueOf:alert + empty string"]
    B3 -->|No| B4{"instanceof<br/>allowed?"}

    B4 -->|Yes| INSTANCE["ES6 well-known symbol<br/>Symbol.hasInstance override"]
    B4 -->|No| B5{"setInterval<br/>allowed?"}

    B5 -->|Yes| SETINT["Timer + tagged template<br/>setInterval with hex-encoded parens"]
    B5 -->|No| B6{"String.replace<br/>allowed?"}

    B6 -->|Yes| REPLACE["Replace callback<br/>String.replace with tagged template"]
    B6 -->|No| B7{"Reflect.apply<br/>allowed?"}

    B7 -->|Yes| REFLECT["Reflect.apply.call<br/>with tagged template args"]
    B7 -->|No| B8{"HTML<br/>context?"}

    B8 -->|Yes| ENTITY["HTML entity parens<br/>lpar + rpar named entities"]
    B8 -->|No| B9{"import<br/>allowed?"}

    B9 -->|Yes| IMPORT["Dynamic import<br/>data: URI with JS payload"]
    B9 -->|No| JSFUCK["JSFuck encoder<br/>6 chars only: #91;#93;#40;#41;!+"]

    style BLOCKED fill:#e74c3c,color:#fff
    style TICK fill:#2ecc71,color:#000
    style THROW fill:#2ecc71,color:#000
    style VALUEOF fill:#2ecc71,color:#000
    style INSTANCE fill:#2ecc71,color:#000
    style SETINT fill:#27ae60,color:#fff
    style REPLACE fill:#27ae60,color:#fff
    style REFLECT fill:#27ae60,color:#fff
    style ENTITY fill:#3498db,color:#fff
    style IMPORT fill:#3498db,color:#fff
    style JSFUCK fill:#9b59b6,color:#fff
```
...
```mermaid
flowchart TD
    NEED["Need JavaScript execution"] --> AUTO{"Need<br/>auto-fire?"}
    ...
    A6 -->|Blocked| A7{"onbeforetoggle?<br/>(Chrome popover)"}
    A7 -->|Blocked| A8{"onafterscriptexecute?<br/>(Firefox only)"}
    A8 -->|Blocked| A9{"accesskey?<br/>(hidden input)"}

    A1 -->|Allowed| E1["img src=x onerror=PAYLOAD"]
    A2 -->|Allowed| E2["svg onload=PAYLOAD"]
    A3 -->|Allowed| E3["details ontoggle=PAYLOAD open"]
    A4 -->|Allowed| E4["input onfocus=PAYLOAD autofocus"]
    A5 -->|Allowed| E5["svg + animate onbegin=PAYLOAD"]
    A6 -->|Allowed| E6["CSS @keyframes + onanimationstart"]
    A7 -->|Allowed| E7["Popover API + onbeforetoggle"]
    A8 -->|Allowed| E8["script tag triggers afterscriptexecute"]
    A9 -->|Allowed| E9["accesskey=X onclick=PAYLOAD<br/>ALT+SHIFT+X to trigger"]

    AUTO -->|No| INT["Interaction-based:<br/>onclick, onmouseover,<br/>onfocusin, ondblclick"]

    style E1 fill:#2ecc71,color:#000
    style E2 fill:#2ecc71,color:#000
    style E3 fill:#2ecc71,color:#000
    style E4 fill:#2ecc71,color:#000
    style E5 fill:#27ae60,color:#fff
    style E6 fill:#27ae60,color:#fff
    style E7 fill:#3498db,color:#fff
    style E8 fill:#3498db,color:#fff
    style E9 fill:#9b59b6,color:#fff
```
...
```mermaid
flowchart LR
    TEST["Test Payload"] --> OBSERVE["Observe Result"]
    OBSERVE --> CLASSIFY{"Classify"}

    CLASSIFY -->|EXECUTED| DONE(["XSS Found"])
    CLASSIFY -->|BLOCKED| LEARN_B["Record:<br/>- Which component triggered block<br/>- WAF response pattern<br/>- Timing information"]
    CLASSIFY -->|REFLECTED| LEARN_R["Record:<br/>- What rendered (footholds)<br/>- What was stripped<br/>- Encoding behavior"]

    LEARN_B --> KB["Knowledge Base"]
    LEARN_R --> KB

    KB --> INFER["Infer:<br/>- Blocked tags set<br/>- Blocked events set<br/>- Blocked keywords set<br/>- Encoding effectiveness<br/>- WAF type"]

    INFER --> COMPOSE["Composer uses ALL<br/>accumulated knowledge<br/>to select next mutation"]
    COMPOSE --> TEST

    style DONE fill:#2ecc71,color:#000
    style KB fill:#3498db,color:#fff
    style INFER fill:#9b59b6,color:#fff
```
...
```mermaid
flowchart TD
    PIVOT["PIVOT: Switch to enumeration"] --> TAGS["Phase 5: Tag Fuzzing<br/>Test 80+ HTML tags across 4 tiers"]

    TAGS --> T1["Tier 1: WAF bypass favorites<br/>body, svg, math, details, select,<br/>textarea, input, button, animate"]
    TAGS --> T2["Tier 2: Standard HTML5<br/>div, span, img, link, form..."]
    TAGS --> T3["Tier 3: Deprecated/obscure<br/>blink, listing, xmp, keygen..."]
    TAGS --> T4["Tier 4: Custom elements<br/>xss, x, foo, custom-tag<br/>(WAFs almost never block these)"]

    T1 --> ALLOWED{"Which tags<br/>passed?"}
    T2 --> ALLOWED
    T3 --> ALLOWED
    T4 --> ALLOWED

    ALLOWED --> EVENTS["Phase 6: Event Fuzzing<br/>For each allowed tag,<br/>test 60+ event handlers"]

    EVENTS --> COMBINE_FINAL["Phase 7: Combine<br/>allowed_tag + allowed_event + allowed_function<br/>→ Test composite payload"]

    COMBINE_FINAL --> EXEC(["XSS or Exhausted"])

    style PIVOT fill:#e74c3c,color:#fff
    style T1 fill:#e76f51,color:#fff
...
flowchart TB
    ...
    subgraph R1["R1: CONTEXT — Where does input land?"]
        direction LR
        CTX_HC["html_content"]
    ...
    BRESULT -->|BLOCKED| BSEARCH["Binary Search<br/>Decompose and test components"]
    BRESULT -->|REFLECTED| BSEARCH

    BSEARCH --> R2

    subgraph R2["R2: BLOCKER — What did we learn is blocked?"]
        direction LR
        BLK_ANG["Angles blocked"]
        BLK_PAR["Parens blocked"]
        BLK_QUO["Quotes blocked"]
        BLK_SPC["Space blocked"]
        BLK_KEY["Keywords blocked"]
        BLK_EVT["Events blocked"]
        BLK_TAG["Tags blocked"]
        BLK_SCR["Script escape"]
        BLK_HRF["javascript: blocked"]
        BLK_WIN["window blocked"]
    end
    ...
    subgraph STRATEGIES["R3+R4: ENCODING + STRUCTURE STRATEGIES"]
        S_ANG["ANGLE<br/>Entity encode #8226; SVG decode<br/>#8226; Attribute context"]
        S_PAR["PAREN<br/>Backticks #8226; throw+onerror<br/>#8226; valueOf #8226; hasInstance<br/>#8226; tagged templates<br/>#8226; Reflect #8226; JSFuck"]
        S_QUO["QUOTE<br/>Switch type #8226; Backtick<br/>#8226; Hex escape #8226; Entities"]
        S_SPC["SPACE<br/>Tab #8226; Newline #8226; Slash<br/>#8226; Formfeed #8226; JS comment"]
        S_KEY["KEYWORD<br/>confirm #8226; prompt #8226; print<br/>#8226; eval+atob #8226; fromCharCode"]
        S_EVT["EVENT<br/>onload #8226; ontoggle #8226; onfocus<br/>#8226; onbegin #8226; onanimationstart<br/>#8226; popover #8226; accesskey"]
        S_TAG["TAG<br/>svg #8226; body #8226; details<br/>#8226; input #8226; math<br/>#8226; custom elements"]
        S_SCR["SCRIPT ESCAPE<br/>Close tag #8226; Backslash double<br/>#8226; Unicode escape"]
        S_HRF["HREF<br/>data: URI #8226; Protocol assign<br/>#8226; Tab in protocol<br/>#8226; navigation.navigate"]
        S_WIN["WINDOW<br/>self #8226; globalThis #8226; this<br/>#8226; top #8226; parent<br/>#8226; defaultView #8226; composedPath"]
    end

    subgraph ADVANCED["ADVANCED STRATEGIES"]
        direction LR
        ADV_CLOB["DOM Clobbering<br/>Override globals via id"]
        ADV_PROTO["Prototype Pollution<br/>__proto__ gadgets"]
        ADV_WNAME["Window.name Smuggling<br/>Cross-origin payload"]
        ADV_SMAP["Source Maps<br/>Exfil via sourceMappingURL"]
        ADV_SVG["SVG Upload<br/>Script in uploaded SVG"]
        ADV_XHTML["XHTML Namespace<br/>XML parsing rules"]
        ADV_FUZZ["Mutation Fuzzing<br/>Double encode, null bytes,<br/>case variation, unicode"]
        ADV_BROWSER["Browser-Specific<br/>Chrome popover,<br/>Firefox afterscriptexecute,<br/>Safari quirks"]
    end

    STRATEGIES --> R5
    ADVANCED --> R5

    subgraph R5["R5: EXECUTION — How does JS fire?"]
        direction LR
        AUTO["Auto-fire<br/>onerror, onload,<br/>ontoggle, onfocus,<br/>onbegin, popover"]
        INTERACT["Interaction<br/>onclick, onmouseover,<br/>accesskey"]
    end
    ...
    TEST -->|EXECUTED| XSS
    TEST -->|BLOCKED| LEARN["Learn new blocker<br/>Update knowledge"]
    LEARN --> R2

    TEST -->|MUTATIONS EXHAUSTED| ENUM["PIVOT: Enumerate<br/>Fuzz 80+ tags #8226; 60+ events<br/>Map filter boundaries"]
    ENUM --> COMBINE

    style XSS fill:#00ff88,stroke:#000,color:#000,stroke-width:3px
    style R1 fill:#264653,stroke:#2a9d8f,color:#fff
    style R2 fill:#2a9d8f,stroke:#e9c46a,color:#fff
    style STRATEGIES fill:#1a1a2e,stroke:#e94560,color:#fff
    style ADVANCED fill:#16213e,stroke:#f4a261,color:#fff
    style R5 fill:#e76f51,stroke:#fff,color:#fff
    style ENUM fill:#f4a261,stroke:#000,color:#000
